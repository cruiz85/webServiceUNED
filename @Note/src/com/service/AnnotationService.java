...
import com.model.service.Activity;
import com.model.service.AnnotationExport;
import com.model.service.Child;
import com.model.service.Taxonomy;
...
public class AnnotationService {
	...
	// UNED
	// Annotated Activities TESTED
	@GET
	@Path("uned/activities/")
	public Activity getAnnotatatedActivities() throws IOException {
		List<Long> list;
		em = EMF.get().createEntityManager();
		Query query = em
				.createQuery("SELECT DISTINCT (g.readingActivity) g FROM Annotation g");
		list = query.getResultList();
		HashSet<Long> hashSet = new HashSet<Long>(list);
		List<Long> ids = new ArrayList<Long>(hashSet);
		Activity activity = new Activity(ids);

		return activity;
	}

	// Annotations By Activities TESTED
	@GET
	@Path("uned/annotations/activity/{activityId}")
	public List<AnnotationExport> getAnnotationsByReadingActivityId(
			@PathParam("activityId") String id) {
		...
		list2 = new ArrayList<Annotation>(list);
		List<AnnotationExport> annotationsExport = buildAnnotationExport(list2);
	...
	private AnnotationExport buildAnnotationExport(
			Annotation annotation) {
			AnnotationExport annotationExport = new AnnotationExport(
					annotation.getId(), annotation.getBookId(),
					annotation.getUserId(), annotation.getPageNumber(),
					annotation.getTextSelectors(), new String(annotation
							.getComment().getValue().replaceAll("\\<.*?\\>","")), annotation.getFileIds(),
					annotation.getReadingActivity());
			
		
		return annotationExport;

	}
	private List<AnnotationExport> buildAnnotationExport(
		...
		return annotationExports;

	}

	// TESTED
	@GET
	@Path("uned/taxonomy/{activity}")
	public ListOfList getAnnotationSchemaByActivityId(
			@PathParam("activity") String id) throws IOException {
		Long activityId = Long.parseLong(id);
		ReadingActivity readingActivity = loadReadingActivityById(activityId);
		ListOfSchema schema = new ListOfSchema();
		ListOfList schemaList = new ListOfList();
		schema = new ListOfSchema(
				getTaxonomyByCatalogId(readingActivity.getCatalogId()));
		schemaList.getListOfList().add(schema);
		return schemaList;

	}

	public List<AnnotationSchema> getTaxonomyByCatalogId(Long catalogId) {
		...
		annotationSchema.setType("C");
		schema.add(annotationSchema);
		for (int j = 0; j < catalogo.getEntryIds().size(); j++) {
			deepingRoot(catalogo.getEntryIds().get(j));
		}

		schema.addAll(buildCatalogueTaxonomy(sonIds));
		return schema;
	}

	private ArrayList<AnnotationSchema> buildCatalogueTaxonomy(
			List<Long> idsDuplicate) {
		HashSet<Long> hashSet = new HashSet<Long>(idsDuplicate);
		List<Long> ids = new ArrayList<Long>(hashSet);
		ArrayList<AnnotationSchema> schema = new ArrayList<AnnotationSchema>();
		AnnotationSchema son = null;
		for (Long id : ids) {
			FolderDB folder = loadFolderById(id);
			if (folder != null) {
				son = new AnnotationSchema(id, folder.getName(),
						folder.getEntryIds(), true);
				son.setType("F");
			} else {
				FileDB file = loadFileById(id);
				son = new AnnotationSchema(id, file.getName(), null, false);
				son.setType("T");
			}
			schema.add(son);
		}
	...
	// Annotation By id
	@GET
	@Path("uned/annotations/{id}")
	public AnnotationExport getAnnotationExportById(@PathParam("id") String id) {
		AnnotationExport annotationExport = buildAnnotationExport(loadAnnotationById(Long.parseLong(id)));
		return annotationExport;
	}
	...
	@GET
	@Path("uned/folder/{id}")
	public FolderDB getFolderById(@PathParam("id") String id) {
		FolderDB folder = loadFolderById(Long.parseLong(id));
		return folder;
	}

	@GET
	@Path("uned/tag/{id}")
	public FileDB getFileById(@PathParam("id") String id) {
		FileDB file = loadFileById(Long.parseLong(id));
		return file;
	}
	...
	public String getHTML(@FormParam("html") String html) throws IOException {
		return html;
	}
	...
	private ReadingActivity loadReadingActivityById(Long id) {
		EntityManager entityManager;
		List<ReadingActivity> list;
		ArrayList<ReadingActivity> listCatalogs;
		entityManager = EMF.get().createEntityManager();
		String sql = "SELECT c FROM ReadingActivity c WHERE c.id=" + id;
		list = entityManager.createQuery(sql).getResultList();
		listCatalogs = new ArrayList<ReadingActivity>(list);
		ReadingActivity readingActivity = listCatalogs.get(0);
